...
class VoyagerBaseController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            $dataTypeContent = $model->withoutGlobalScopes()->findOrFail($id);
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isSoftDeleted'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($model && in_array(SoftDeletes::class, class_uses_recursive($model))) {
            $data = $model->withoutGlobalScopes()->withTrashed()->findOrFail($id);
        } else {
            $data = $model->withoutGlobalScopes()->findOrFail($id);
        }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        foreach ($ids as $id) {
            $data = $dataType->model->withoutGlobalScopes()->findOrFail($id);
        ...
        $data = (isset($res) && $res > 0) || (isset($deleteStatus) && $deleteStatus === true)
            ? [
    ...
    /**
     * This method allows us to toggle a bread column
     * For exemple we need to switch between product
     * Availability id, in stock or, out of stock
     *
     * @param  Request $request
     * @param  int     $id
     * @return Response
     */
    public function toggleColumn(Request $request, int $id, string $column)
    {
        $slug = $this->getSlug($request);
        ...
        $editableColumns = $dataType->editRowsInputs->pluck('field')->toArray();

        if ($dataType->model_name == Product::class) {
            $editableColumns[] = 'status';
            $editableColumns[] = 'availability_id';
            $editableColumns = array_unique($editableColumns);
        }
        ...
        $index = array_search($request->input('value'), $request->input('options'));
        ...
        $newValue = $request->input('values')[$index] ?? null;

        $model = $dataType->model->withoutGlobalScopes()->findOrFail($id);
    ...
    public function remove_media(Request $request)
    {
        try {
            ...
            $model = app($dataType->model_name);
            $data = $model::find([$id])->withoutGlobalScopes()->first();
    ...
    public function update_order(Request $request)
    {
        ...
        foreach ($order as $key => $item) {
            if ($model && in_array(SoftDeletes::class, class_uses_recursive($model))) {
                $i = $model->withTrashed()->withoutGlobalScopes()->findOrFail($item->id);
            } else {
                $i = $model->withoutGlobalScopes()->findOrFail($item->id);
            }
            $i->$column = ($key + 1);
            $i->save();
        }
    ...
    public function relation(Request $request)
    {
        ...
        $model = app($dataType->model_name);
        if ($method != 'add') {
            $model = $model->withoutGlobalScopes()->find($request->input('id'));
        }
