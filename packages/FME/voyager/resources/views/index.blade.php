...
@section('content')
    <div class="page-content px-2">
        ...
        <div class="container-fluid" id="analytics">
            ...
            <div class="row" v-if="analytics !== null">

                <div class="col-2">
                    <analytics-box :today="analytics.today_orders.today" :yesterday="analytics.today_orders.yesterday" type="number" style="height: 60px;">
                        Today's orders
                    </analytics-box>
                </div>

                <div class="col-2">
                    <analytics-box :today="analytics.today_sales.today" :yesterday="analytics.today_sales.yesterday" type="money" style="height: 60px;">
                        TODAY'S SALES
                    </analytics-box>
                </div>

                <div class="col-2">
                    <analytics-box :today="analytics.today_avg_sales.today" :yesterday="analytics.today_avg_sales.yesterday" type="money" style="height: 60px;">
                        TODAY'S average sales
                    </analytics-box>
                </div>

                <div class="col-2">
                    <analytics-box :today="analytics.all_time.today" :yesterday="analytics.all_time.yesterday" type="money" style="height: 60px;">
                        All time sales
                    </analytics-box>
                </div>

                <div class="col-2">
                    <analytics-box :today="analytics.all_time_avg_sales.today" :yesterday="analytics.all_time_avg_sales.yesterday" type="money" style="height: 60px;">
                        All time average sales
                    </analytics-box>
                </div>

                <div class="col-2">
                    <div :class="isCheckoutBug ? 'bg-dark-danger' : 'bg-dark'" class="card border-0 rounded-lg shadow-none position-relative d-flex flex-row" style="height: 60px;">
                        <div class="d-flex flex-fill p-2 text-white position-relative align-items-center justify-content-between">
                            <div class="text-uppercase pl-2">
                                <h1 style="font-size: 9px;" class="small mb-0 font-weight-light text-nowrap">Completed / abandoned orders</h1>
                                <div class="d-flex py-0 flex-row align-items-end">
                                    <h2 class="mb-0 h4" style="line-height: 1.7rem;">
                                        <code class="text-white font-weight-light text-nowrap">
                                            @{{ completed }}
                                        </code>
                                    </h2>
                                    <span class="h4 font-weight-light mb-0 mx-1">-</span>
                                    <h2 class="mb-0 h4 font-weight-light">
                                        <code class="text-soft-danger font-weight-light text-nowrap">
                                            @{{ abandoned }}
                                        </code>
                                    </h2>
                                </div>
                        ...
                        <div 
                            :class="isCheckoutBug ? 'bg-dark-danger text-soft-danger' : 'bg-dark-2 text-dark'" 
                            class="p-0 rounded-lg d-flex flex-row align-items-center justify-content-center" 
                            style="height: 60px; width: 60px; margin: auto; text-align: center;"
                        >
            ...
            <div class="row pt-4 mt-4 mb-4 align-items-start justify-content-end">
                <div class="col">
                    <div class="border border-secondary bg-light p-4 rounded-lg w-100 mr-5">
                        <h1 class="h5 text-uppercase mb-3">Top 10 products sold</h1>
                            ...
                            <tbody>
                                @foreach ($salesProducts as $product)
                                    ...
                                    @if ($product instanceof \App\Product)
                                        <tr>
                                            <td>
                                                <a target="_blank" href="{{ route('product.show', $product) }}">{{ $product->id }}</a>
                                            </td>
                                            <td class="text-nowrap">{{ $product->sku }}</td>
                                            <td class="text-nowrap">
                                                <a href="#" class="text-dark" data-toggle="tooltip" title="{{ $product->name }}">{{ \Illuminate\Support\Str::limit($product->name, 27) }}</a>
                                            </td>
                                            <td>{{ $product->sales_count }}</td>
                                            <td>{{ $product->clicks_counter }}</td>
                                        </tr>
                                    @endif
                ...
                <div class="col">
                    <div class="border border-secondary bg-light p-4 rounded-lg w-100 mr-5">
                        <h1 class="h5 text-uppercase mb-3">Top 10 products viewed</h1>
                        <table class="table table-sm table-hover table-striped mb-0">
                            <thead class="thead-dark">
                                <tr>
                                    <th>ID</th>
            ...
            <div class="row pt-4 mt-4 mb-5 align-items-start justify-content-end">
                <div class="col">
                    <div class="border border-secondary bg-light p-5 rounded-lg">
                    ...
                    <div class="border border-secondary bg-light p-5 rounded-lg">
                        <div class="row">
                            <div class="col">
                                <div class="">
                                    <div class="mb-3">
                                        <h1 class="h5 text-uppercase mb-0">Products Sales</h1>
                                        <p class="mb-0 text-muted">Total products sold on this period</p>
                                    </div>
                                    <div class="p-3">
                            ...
                            <div class="col">
                                <table class="table table-sm table-hover table-striped mb-0">
                                    ...
                                    <tbody>
                                        @foreach ($productsChart->getProductsData() as $data)

                                            @if ($data->product instanceof \App\Product && isset($data->product->id))
                                                <tr>
                                                    <td>
                                                        <a target="_blank" href="{{ route('product.show', $data->product) }}">{{ $data->product->id ?? 'N/A' }}</a>
                                                    </td>
                                                    <td class="text-nowrap">{{ $data->product->sku ?? '' }}</td>
                                                    <td class="text-nowrap">
                                                        <a 
                                                            href="#" 
                                                            class="text-dark" 
                                                        ...
                                                        >
                                                                {{ \Illuminate\Support\Str::limit($data->product->name ?? '', 27) }}
                                                        </a>
                                                    </td>
                                                    <td>{{ $data->sales ?? '' }}</td>
                                                    <td>{{ (int) $data->product->clicks_counter ?? '' }}</td>
                                                </tr>
                                            @endif
        ...
        <div class="analytics-container">
            ...
            @php
                $google_analytics_client_id = Voyager::setting("admin.google_analytics_client_id");
            @endphp
            ...
            methods: {
                getData() {
                    ...
                    .done(function(response) {
                        self.abandoned = response.abandoned
                        self.completed = response.completed
                        self.analytics = response.analytics
                        self.isCheckoutBug = response.isCheckoutBug
                    })
                    .fail(function() {
                        console.log("error");
                    })
